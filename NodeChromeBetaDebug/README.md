...
[`selenium/node-chrome-beta-debug-debug` Dockerfile](https://github.com/SeleniumHQ/docker-selenium/blob/master/NodeChromeBetaDebug/Dockerfile)
...
```
$ docker run -d -P --link selenium-hub:hub selenium/node-chrome-beta-debug-debug
```
...
``` dockerfile
FROM selenium/node-chrome-beta-debug-debug:3.4.0-chromium
